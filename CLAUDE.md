...
## TDD Contract
NEVER write implementation code before a failing test exists for it. This is blocking.
Sequence (no exceptions): RED (write failing test, confirm it fails) → GREEN (minimum code to pass) → REFACTOR.
PostToolUse hooks auto-run tests on every save. Stop hook blocks exit if tests are red.
...
```
write-failing-test (RED) → implement (GREEN) → /write-tests → /review-code → fix P0/P1 → /create-pr
...
## ADR Index
All architectural decisions live in `docs/adr/`. Read `docs/adr/INDEX.md` for the decision index before proposing any change to storage, API design, frontend patterns, or data models. The index is auto-generated — never edit it manually.
